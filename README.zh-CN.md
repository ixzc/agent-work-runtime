...
本次 Apple M3 Max / macOS 上，预热后连续调用 30 次，上下文编译 **p95 为 108 毫秒**。这是单机测量，不代表并发容量。

当前源码还增加了[完整工作流程对照](docs/benchmarks/workflow.md)：30 次合成流程保持同等完成合同，工具调用减少 **18%–27%**，返回文本仅减少 **3.4%–4.7%**。台账维护仍有成本；这不等于实际模型 token 或费用降幅，也不是新版本发布声明。
